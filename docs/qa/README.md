...
| `lighthouse.mjs` (`[/seite …]`, `--mobile|--desktop`, `--seo`, `--full`) | Lighthouse für alle Seiten, mobil und Desktop; Gate ≥ 90 für Performance, Accessibility, Best Practices (SEO mitgemessen, Gate erst ab Phase 6b mit `--seo`); Report `docs/qa/lighthouse/<datum>/summary.md`, Einzelberichte (HTML + JSON, ~1 MB je Messung) nur mit `--full` und nicht committen | Phase 6 ff. |
